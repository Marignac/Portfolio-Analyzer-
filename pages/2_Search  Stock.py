...
# Display the logo
    ...
    # Use session state if query param isn't available
    selected_stock = st.session_state.get("selected_stock", "Please select a stock")
    ...
    # Preselect stock in dropdown
    stock_options = ["Please select a stock"] + [f"{row['Name']} ({row['Ticker']})" for _, row in final_df.iterrows()]
    try:
        selected_stock_index = stock_options.index(selected_stock)
    except ValueError:
        selected_stock_index = 0

    selected_stock_dropdown = st.sidebar.selectbox(
        "Search for a stock:", stock_options, index=selected_stock_index
    )

    if selected_stock_dropdown == "Please select a stock" or not selected_stock_dropdown:
        st.title("Please select a stock")
    else:
        # Extract the selected stock data
        stock_name = selected_stock_dropdown.split(" (")[0]
        stock_row = final_df[final_df['Name'] == stock_name]
        st.title(stock_name)
